...
```bash
# 启动（防止串到其它 venv：用 venv 全路径 python，且不要加 --reload）
backend/.venv/Scripts/python.exe -m uvicorn app.main:app --port 8000 --app-dir backend
# 健康检查
...
1. **必须走代理**：本机 Clash(verge-mihomo) 常驻 + TUN 模式，直连高德/DeepSeek 会 TLS 失败。`backend/.env` 的 `PROXY_URL=http://127.0.0.1:7897` 必须保留；`main.py` 启动时据此注入 `HTTP(S)_PROXY`。换到国内真直连环境才清空。详见 memory `route-system-env`。
