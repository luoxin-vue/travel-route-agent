...
export function Button({ variant = "primary", className = "", children, ...rest }: Props) {
  const base =
    "inline-flex items-center justify-center gap-2 rounded px-4 py-2 text-code-md font-mono transition-colors disabled:opacity-40 disabled:cursor-not-allowed";
  const styles = {
...
export function CommandButton({ children, className = "", ...rest }: ButtonHTMLAttributes<HTMLButtonElement> & { children: ReactNode }) {
  return (
    <button
      className={`font-mono text-label-sm text-on-surface-variant hover:text-primary-container transition-colors ${className}`}
      {...rest}
    >
      [{children}]
    </button>
