...
1. **Prioritize CodeGraph Tools**: Always prefer calling CodeGraph tools (such as `codegraph_explore` or other `mcp__codegraph__*` tools) over standard file/text search tools (like `grep_search`, `list_dir`, or sequentially opening files via `view_file` to find where symbols are defined or called).
