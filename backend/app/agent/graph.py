"""构建 LangGraph Multi-Agent 编排：主管 Agent + 交通专家 + 住宿专家。"""
...
from app.agent.prompts import SUPERVISOR_PROMPT, TRANSIT_PROMPT, LODGING_PROMPT
...
async def get_agent():
    """惰性构建并缓存 Multi-Agent 编排图（首次拉取 MCP 工具为异步）。"""
    global _agent
    if _agent is None:
