...
class ItineraryNode(BaseModel):
    ...
    lng: Optional[float] = Field(default=None, description="经度（高德为 GCJ-02，国际为 WGS-84）")
    lat: Optional[float] = Field(default=None, description="纬度（高德为 GCJ-02，国际为 WGS-84）")
    ...
    image: Optional[str] = Field(
        default=None,
        description="该地点的真实照片 URL（国内取自高德 photo 字段，国际取自 Wikipedia/Wikimedia）；没有则留空，不要编造",
    )
    next_distance_km: Optional[float] = Field(
        default=None,
        description="到下一个 stop 节点的路线距离（公里），通过高德 distance 或 intl_distance 工具获取",
    )
...
class Itinerary(BaseModel):
    ...
    cover_image: Optional[str] = Field(
        default=None,
        description="行程封面图 URL（国内取自高德 photo，国际取自 Wikimedia）；只用工具返回的真实 URL，不要编造",
    )
